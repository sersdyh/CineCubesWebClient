...
public class HighlightDominationRow extends Highlight {

	public ArrayList<String> helpValues2;
	
	public HighlightDominationRow() {
		super();
	...
	public void execute(String[][] PivotTable){
    	...
    	int min_index=1;
    	int middle_index=2;    
    	DominationColor=new Integer[PivotTable.length][3];
    			...
    			if(in_middle){
    				for(int k=0;k<helpValues2.size();k++){
    					if(Float.parseFloat(helpValues2.get(k))==currentValue){
    						DominationColor[i][min_index]++;
    	    				valuePerColor[min_index]++;
		    				in_middle=false;
		    			}
	    			}
	    			if(in_middle){
	    				DominationColor[i][middle_index]++;
